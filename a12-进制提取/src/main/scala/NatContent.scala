...
object 自然数容器I {
  ...
  /*implicit class 自然数容器I扩展[Tail <: 自然数, Head, TagPair <: 自然数标记](private val 自然数容器I: 自然数容器I[正整数[Tail, Head], TagPair, 正整数自然数对[Tail, Head]]) extends AnyVal {
    def get: Head = 自然数容器I.natPair.正数部分.head
  }*/

  def apply[Pair <: 自然数, TagPair <: 自然数标记](数对: Pair, 数标记: TagPair): 自然数容器I[Pair, TagPair] = new 自然数容器I(数对 = 数对, 数标记 = 数标记)
