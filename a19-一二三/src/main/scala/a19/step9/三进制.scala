...
class 上层_1[T1, T2, II1 <: 三进制后继, II2 <: 三进制后继] extends 三进制后继 {
  ...
  override type Next[T] = II1#是否进位[上层_1[T1, T2, II1#Next[T], II2], 上层_2[T1, T2, II1, II2#Next[T]]]
}
...
class 上层_2[T1, T2, II1 <: 三进制后继, II2 <: 三进制后继] extends 三进制后继 {
  ...
  override type Next[T] = II2#是否进位[上层_2[T1, T2, II1, II2#Next[T]], 上层_1[T1, T2, II1#开辟#Next[T], II2#开辟]]
}
...
class 底层_2[T0, T1, T2] extends 三进制后继 {
  ...
  override type 开辟                              = 底层_2_待进位[T0, T1, T2]
}

class 底层_2_待进位[T0, T1, T2] extends 三进制后继 {
  type _0_th = T0
  type _1_th = T1
  type _2_th = T2
...
class 中层满位_0[T0, T1, T2, II0 <: 三进制后继, II1 <: 三进制后继, II2 <: 三进制后继] extends 三进制后继 {
  ...
  override type Next[T]                         = II0#是否进位[中层满位_0[T0, T1, T2, II0#Next[T], II1, II2], 中层满位_1[T0, T1, T2, II0, II1#Next[T], II2]]
  override type 是否进位[不进位 <: 三进制后继, 进位 <: 三进制后继] = 不进位
}
...
class 中层满位_1[T0, T1, T2, II0 <: 三进制后继, II1 <: 三进制后继, II2 <: 三进制后继] extends 三进制后继 {
  ...
  override type Next[T]                         = II1#是否进位[中层满位_1[T0, T1, T2, II0, II1#Next[T], II2], 中层满位_2[T0, T1, T2, II0, II1, II2#Next[T]]]
  override type 是否进位[不进位 <: 三进制后继, 进位 <: 三进制后继] = 不进位
}
...
class 中层满位_2[T0, T1, T2, II0 <: 三进制后继, II1 <: 三进制后继, II2 <: 三进制后继] extends 三进制后继 {
  ...
  override type 开辟                              = 中层满位_0[T0, T1, T2, II0#开辟, II1#开辟, II2#开辟]
}
