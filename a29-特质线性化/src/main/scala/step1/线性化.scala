...
object 自然数Positive extends 自然数Positive2 {
  implicit def n[T <: 自然数, H]: 自然数Func[自然数Positive[T, H], H, ({ type T[I <: 自然数, _] = I })#T] =
    new 自然数Func[自然数Positive[T, H], H, ({ type T[I <: 自然数, _] = I })#T]
  implicit def n3[T <: 自然数, T2 <: 自然数, H, I[_ <: 自然数, _] <: 自然数](implicit
    k: 自然数Func[T, H, I],
    k2: 自然数IO[T, T2]
  ): 自然数IO[自然数Positive[T, H], I[T2, H]] =
    new 自然数IO[自然数Positive[T, H], I[T2, H]]
}
trait 自然数Positive2 {
  implicit def n2[T <: 自然数, H, H1, II[_ <: 自然数, _] <: 自然数](implicit i: 自然数Func[T, H1, II]): 自然数Func[自然数Positive[T, H], H1, II] =
    new 自然数Func[自然数Positive[T, H], H1, II]
}
