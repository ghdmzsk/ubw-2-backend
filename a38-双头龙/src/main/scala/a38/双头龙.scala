...
case class Number4_Middle_1_In(tail: Number1) extends Number1 {
  override def value: Value = {
    val lit = "0" + tail.value.literal
    Value(value = tail.value.value, literal = lit)
  }
...
object Number5 {
  def len(num: Number1): Int = num match {
    ...
    case Number4_Middle_1_In(tail) => len(tail) + 1
  ...
  def method1(number1: Number1): Number1 = {
    number1 match {
      case Number3_1(tail) =>
        method1(tail) match {
          case Number4_Middle_1_In(tail) => Number3_2(tail)
          case i                         => Number3_1(i)
        }
      case Number3_2(tail) =>
        method1(tail) match {
          case i @ Number4_Middle_1_In(tail) => Number3_1(i)
          case i                             => Number3_2(i)
        }
      case Number4_Middle_1(tail) =>
        method1(tail) match {
          case Number4_Middle_1_In(tail) => Number4_Middle_2(tail)
          case i                         => Number4_Middle_1(i)
        }
      case Number4_Middle_2(tail) =>
        method1(tail) match {
          case Number4_Middle_1_In(tail) => Number4_Middle_3(tail)
          case i                         => Number4_Middle_2(i)
        }
      case Number4_Middle_3(tail) =>
        method1(tail) match {
          case i @ Number4_Middle_1_In(tail) => Number4_Middle_1_In(i)
          case i                             => Number4_Middle_3(i)
        }
      case Number4_Middle_1_In(tail) => Number4_Middle_1(method1(tail))
      case Number4_Bottom_1          => Number4_Bottom_2
      case Number4_Bottom_2          => Number4_Bottom_3
      case Number4_Bottom_3          => Number4_Middle_1_In(Number4_Bottom_1)
    }
  }
