...
object NumberT1 {
  def fromInt(n: Int): NumberT1 = {
    ...
    @tailrec
    def getZero(numberT1: NumberT1): NumberT1 = {
      numberT1 match {
        case a: NumberT1Positive => getZero(a.tail)
        case b: NumberT1Zero     => b
      }
    }
    getZero(nn)
  }
...
object NumberT2 {
  def fromInt(n: Int): NumberT2 = {
    def fromIntImpl(n1: Int, p: () => NumberT2): NumberT2 = {
      n1 match {
        case n2 if n2 > 0 => new NumberT2Positive(fromIntImpl(n1 - 1, p))
        case 0            => new NumberT2One(p)
      }
    }
