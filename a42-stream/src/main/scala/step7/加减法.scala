...
case class RNumB(right: NumRight, sub: CTree) extends BTree {
  def 向右获取(method: Method): Result = right.被索取(RNumMethodC(method = method, sub = sub))
}
...
trait CTree {
  def 反馈元素(method: NumRight, item: Item): Result
...
case class LTreeC(left: ATree, method: Method, sub: CTree) extends CTree {
  override def 反馈元素(num: NumRight, item: Item): Result = sub.反馈上层(LTreeRNumA(left = left, method = method, right = num), item)
...
case class LNumC(left: NumLeft, method: Method, sub: CTree) extends CTree {
  override def 反馈元素(num: NumRight, item: Item): Result = sub.反馈上层(LNumRNumA(left = left, method = method, right = num), item)
...
case class RNumMethodC(method: Method, sub: CTree) extends CTree {
  override def 反馈元素(num: NumRight, item: Item): Result = sub.反馈上层(RNumA(method = method, right = num), item)
  ...
  override def 结束反馈: Result                            = sub.结束反馈
}
