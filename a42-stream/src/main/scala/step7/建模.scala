...
case class LTreeRTreeB(left: ATree, right: ATree, sub: CTree) extends BTree {
  override def 向右获取(method: MethodRight): Result          = right.被索取(LTreeC(left = left, method = method, sub = sub))
  ...
  override def 向左获取(method: MethodLeft): Result           = left.被索取(RTreeC(right = right, method = method, sub = sub))
}
case class LNumRTreeB(left: Num, right: ATree, sub: CTree) extends BTree {
  override def 向右获取(method: MethodRight): Result          = right.被索取(LNumC(left = left, method = method, sub = sub))
...
case class LTreeRNumB(left: ATree, right: Num, sub: CTree) extends BTree {
  ...
  override def 向左获取(method: MethodLeft): Result           = left.被索取(RNumC(right = right, method = method, sub = sub))
}
...
case class LTreeC(left: ATree, method: MethodRight, sub: CTree) extends CTree {
...
case class LNumC(left: Num, method: MethodRight, sub: CTree) extends CTree {
...
case class RTreeC(right: ATree, method: MethodLeft, sub: CTree) extends CTree {
...
case class RNumC(right: Num, method: MethodLeft, sub: CTree) extends CTree {
