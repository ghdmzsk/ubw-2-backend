...
object Runner2 {
  ...
  def main(arr: Array[String]): Unit = {
    {
      val number1                        = number1gen(1888)
      val (number2Positive, number2Zero) = 除数(4)
      val result: Number1                = Log.logNumber(number1, number2Positive)
      val countResult                    = Log.log(4, 256)
      println(count(result))
      println(Log.log(4, 1888))
    }
    {
      val number1                        = number1gen(10)
      val (number2Positive, number2Zero) = 乘数(2)
      val result: Number1                = Pow.powNumber(number1, number2Positive)
